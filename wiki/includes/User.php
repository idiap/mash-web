...
class User {
	...
	function isAllowed( $action = '' , $title=NULL) {
		...
		// For api to work with hiddenwiki
		if (is_object($title)) {
		...
		// If user has the edit right, allow him to edit pages in the restricted namespaces
		// he has access.
		if( is_array($wgRestrictedNamespaces) && array_key_exists($ns, $wgRestrictedNamespaces)
			...
			// If user has the sprotect right, allow him to edit pages in the restricted namespaces
			// he has access.
			if(($action == 'protect' || $action == 'unprotect') && in_array('sprotect', $this->mRights)) {
				return true;
			}

			// If user has the sdelete right, allow him to edit pages in the restricted namespaces
			// he has access.
	...
	function canAccessNamespace( $namespace='' ) {
		...
		if( is_array( $wgRestrictedNamespaces )) {
			if( array_key_exists( $namespace, $wgRestrictedNamespaces ) ) {
				if( in_array($wgRestrictedNamespaces[$namespace], $this->getRights())) {
					return true;
				}
				return false;
			}
	...
	function getAllowedRNSes() {
		...
		if( is_array( $wgRestrictedNamespaces ) ) {
			foreach ($wgRestrictedNamespaces as $nsid => $ugroup) {
				#This line shows only every second namespace in the sidebar, thus removing potential
				#discussionpages from the bar.
				if( $this->canAccessNamespace($nsid) && !($nsid %2) ) {
				#This line shows all hidden pages in the sidebar. If you uncomment this make sure you
				#comment out the above line.
				# if( $this->canAccessNamespace($nsid) ) {
                    if (!$first)
                        $names .= "\n";

					$names .= MWNamespace::getCanonicalName($nsid);
