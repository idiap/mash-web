    ...
    function init(plot) {
        function bindEvents(plot, eventHolder) {
            ...
            if (o.pan.interactive) {
                var prevCursor = 'move', pageX = 0, pageY = 0;
        ...
        plot.zoom = function (args) {
            ...
            if (args.out)
                plot.currentZoom -= 0.5;
            else
                plot.currentZoom += 0.5;
            
            if (plot.currentZoom < 1.0)
                plot.currentZoom = 1.0;
            
            var dx = (options.xaxis.panRange[1] - options.xaxis.panRange[0]) / (2.0 * plot.currentZoom);
            var dy = (options.yaxis.panRange[1] - options.yaxis.panRange[0]) / (2.0 * plot.currentZoom);
            
            var pc = { left: axes.xaxis.c2p(c.left), top: axes.yaxis.c2p(c.top) };
            
            if (pc.left < options.xaxis.panRange[0] + dx)
                pc.left = options.xaxis.panRange[0] + dx;
            else if (pc.left > options.xaxis.panRange[1] - dx)
                pc.left = options.xaxis.panRange[1] - dx;

            if (pc.top < options.yaxis.panRange[0] + dy)
                pc.top = options.yaxis.panRange[0] + dy;
            else if (pc.top > options.yaxis.panRange[1] - dy)
                pc.top = options.yaxis.panRange[1] - dy;

            options.xaxis.min = pc.left - dx;
            options.xaxis.max = pc.left + dx;
            options.yaxis.min = pc.top - dy;
            options.yaxis.max = pc.top + dy;
